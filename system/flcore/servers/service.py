...
class ServicoSelecaoClientes:
    ...
    def exibir_resumo_selecao(self, todos_clientes, top_k, aleatorios, medias_ponderadas=None):
        ...
        for c in todos_clientes:
            ...
            acc_bruta_colored.append(f"{cor}{c.test_accuracy:^{self.col_width}.3f}{self.RESET}")
            acc_penalizada_colored.append(f"{cor}{c.test_accuracy:^{self.col_width}.3f}{self.RESET}")
            ...
            if medias_ponderadas and c.id in medias_ponderadas:
                ...
                medias_colored.append(f"{cor}{media:^{self.col_width}.1f}{self.RESET}")
            else:
